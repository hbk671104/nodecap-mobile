...
@connect(({ institution_create, loading }, { navigation }) => {
  const index = navigation.getParam('index', 0);
  return {
    member: R.path(['current', 'members', index])(institution_create),
    loading: loading.effects['institution_create/editMember'],
  };
})
@createForm({
  ...
  handleSavePress = () => {
    this.props.form.validateFields((err, value) => {
      if (!err) {
        ...
        this.props.dispatch({
          type: 'institution_create/editMember',
          id: member.id,
          payload: value,
          callback: success => {
            if (success) {
