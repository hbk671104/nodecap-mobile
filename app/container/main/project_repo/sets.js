    ...
    this.props.showActionSheetWithOptions({
      ...
      if (index !== 2) {
        const request = {
          ...
          thumbImage: 'https://hotnode-production-file.oss-cn-beijing.aliyuncs.com/coin-set-share.jpg',
        };
        if (index === 0) {
  ...
  renderHeader = () => {
    return (
      <Flex style={styles.header} align="center" justify="between">
        <Text style={styles.headerText}>{this.props.tabLabel}为您精选 <Text style={styles.headerHighlight}>{R.path(['pagination', 'total'])(this.props)}</Text> 个优质项目 </Text>
        <Flex align="center">
          <Image style={{ width: 13, height: 13 }} source={require('asset/coin_sets/share_active.png')} />
          <TouchableWithoutFeedback onPress={this.onPressShare}>
            <View>
              <Text style={{ marginLeft: 7, fontSize: 14, color: 'rgba(0,0,0,0.65)', letterSpacing: 0.17 }}>分享</Text>
            </View>
